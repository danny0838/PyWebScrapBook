...
* Reworked `limit:` command to list the top N items across all queried scrapbooks.
* Miscellaneous improvements to the internal code and test suite.

## [2.7.2] - 2025-09-06
* Added cross-platform tests in other Python implementations.
* Fixed some issues in PyPy.
* Miscellaneous improvements and bug fixes to the workflows and test suite.

## [2.7.1] - 2025-09-03
* Added automatic cross-platform test workflow.
* Miscellaneous improvements to the dev tools and workflows.
...
* Miscellaneous improvements to the internal API and the unittest suite.

## [2.4.1] - 2024-10-23
* Fixed CSS handling error for certain `url()` when converting an item to single_file format.
* Miscellaneous improvements to the unittest suite.

## [2.4.0] - 2024-07-28
* Fixed an error on an invalid URL during generating fulltext cache.
* Fixed an issue where inproper attributes of the root element were taken by the checker/indexer.
* Fixed an issue where a self-extracting HTML file generated by SingleFile was not correctly processed by the file2wsb utility.
* Improved support for XHTML and SVG files for the checker/indexer.
* Improved handling of legacy `data-scrapbook-shadowroot` attribute for the migration utility.
* Miscellaneous improvements and bug fixes to the source code and the unittest suite.
...
## [2.1.0] - 2024-01-19
* Added support for opening an archive file using the `wsb` executable.
...
* Added support for `book.*.new_at_top` config.
...
## [1.13.2] - 2023-05-17
* Fixed an issue where importing an archive file with '*/index.html' as index got a bad cached icon path.

## [1.13.1] - 2023-05-10
* Fixed an issue where importing archive files of a multi-referenced item using "new" mode got duplicated items.
* Improved the documentation about the `export`/`import` command.

## [1.13.0] - 2023-05-08
* Fixed an issue where `*/index.html` were always added after `*.htz`, `*.maff`, `*.html` in the same directory when adding unindexed files by the checker.
* Fixed an issue where a support folder might be incorrectly handled on a case sensitive filesystem when checking for unindexed files.
* Fixed an issue where the icon property might be incorrect when importing an item into a different book.
* Miscellaneous optimization and refactoring of the source code.

## [1.12.2] - 2023-05-07
* Fixed an issue where an error message was shown when caching an empty web page.
* Fixed an issue where an exported archive with a bad version was not rejected on import.
...
* Fixed an issue where downoloading a collection in a ZIP may got a bad filename.
* Fixed an issue where a file or directory could be unrestrictedly created at a subpath of a file in a ZIP.
* Fixed an issue where a directory could be unrestrictedly moved to a path under self and causes a further error in some cases.
* Fixed an issue where permissions were not preserved when copying a directory into a ZIP.
* Fixed an issue where metadata were not preserved when modifying a file in a ZIP.
...
## [1.8.1] - 2023-01-20
* Fixed an issue where the `.wsb` directory under the `top_dir` of a scrapbook was not ignored when running the checker.
...
* Fixed an issue where some control chars in a filename were not correctly handled.
* Fixed an issue where ZIP comment might be missing after a modification of its content.
...
* Changes of Backend server API:
  ...
  * Fixed an issue where a path like "/", "foo//bar", "foo/./bar", "foo/../bar", etc., was sometimes not tidied before further processing.
...
* Improved the default theme of the web interface:
  ...
  * Added support for selection and commands in all view modes.
  * Added a hint for current selection count.
  * Added support for responsive media sizing for gallery view mode.
  ...
  * Added support for moving/copying/linking a single entry at a folder.
...
* Improved the default theme of the web interface:
  ...
  * Added support for uploading files and directories through drag-and-drop.
...
## [1.1.0] - 2021-12-10
* Dropped support for the `app.base` config.
* Fixed an issue where the mimetype of `.js` became `application/x-javascript` in some environments.
* Fixed several unit test errors.
...
## [0.45.0] - 2021-10-25
* Added support for HTZ, MAFF, and single HTML formats for the `migrate` converter.
...
## [0.44.0] - 2021-05-08
* Added support for `single_file` format for `items` converter.
* Added support for more common MIME types.
...
## [0.43.0] - 2021-05-04
* Added support for custom MIME types mapping using `~/.config/wsb/mime.types`.
* Added `wsb help mimetypes` for documentation about customizing MIME types mapping.
* Added support for some common MIME types.
* Reworked handling of some HTTP headers and HTML attributes to conform with the spec better.
* Fixed an issue where meta charset was ignored when parsing a meta refresh.
...
## [0.42.0] - 2021-05-01
* Added support for migrating several older WebScrapBook data for the `migrate` converter. The behavior can be switched with `--convert-*` options.
* `migrate` converter no more changes the linefeed format for output files.
* Adjusted log message format of several utilities.
* Fixed missing support for hash in source URL for the generated static index file.
...
* Added support for `limit:` command for search page.
...
* Fixed an issue where `file2wsb` converter did not handle a page named `index.html` with a support folder.
...
* `file2wsb` converter now preserves the original filename by default, with an added `--no-preserve-filename` option to configure the behavior.
...
* Added support for downloading a folder or files and folders under a folder for the web interface.
* Added `--data-folder-suffixes` and `--ignore-*-meta` options for `file2wsb` converter.

## [0.36.0] - 2021-04-01
* Fixed an issue where auto backup might not include a deleted file.
* Added support for note for a backup.
* Added support for `unbackup` action.

## [0.35.2] - 2021-03-30
* Fixed an issue where backup did not work for `sb2wsb` converter if input path was not absolute.

## [0.35.0] - 2021-03-27
* Fixed an issue where fulltext cache did not work for iframes with srcdoc attribute. (Rebuild the fulltext cache to correct existing caches.)
* Added support for page conversion for `sb2wsb` and `wsb2sb` converters.
...
* Added support for top-level None value for *.js tree files.
...
## [0.11.0] - 2020-01-14
* Added support for zip editing through server actions.
